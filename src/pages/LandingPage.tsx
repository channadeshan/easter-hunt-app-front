...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-5 relative">
      {/* Decorative blobs */}
      <div
        className="fixed top-0 left-0 w-72 h-72 rounded-full opacity-20 blur-3xl"
        style={{
          background: "radial-gradient(circle, #ff6b6b 0%, transparent 70%)",
        }}
      />
      <div
        className="fixed bottom-0 right-0 w-72 h-72 rounded-full opacity-20 blur-3xl"
        style={{
          background: "radial-gradient(circle, #cc5de8 0%, transparent 70%)",
        }}
      />
      <div
        className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 rounded-full opacity-10 blur-3xl"
        style={{
          background: "radial-gradient(circle, #ffd43b 0%, transparent 70%)",
        }}
      />
          ...
          <div className="space-y-3 animate-slide-up stagger-1">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="ml-auto text-gray-600 group-hover:text-hunt-canary transition-colors">
                  →
                </span>
              </div>
            </button>
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-hunt-lavender/10 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                  🔐
                </div>
                <div>
                  <p className="font-display font-semibold text-white text-base">
                ...
                <span className="ml-auto text-gray-600 group-hover:text-hunt-lavender transition-colors">
                  →
                </span>
              </div>
            </button>
        ...
        {mode === "admin" && (
          <form onSubmit={handleAdminLogin} className="animate-slide-up">
            <div className="glass rounded-2xl p-5 border-hunt-border">
              ...
              <label className="block text-xs text-gray-500 uppercase tracking-widest mb-2 font-body">
                Password
              </label>
              <input
                type="password"
              ...
              />
              {error && (
                <p className="text-hunt-coral text-xs mt-2 font-body">
                  {error}
                </p>
              )}
              <button
        ...
        {mode === "player" && (
          <form onSubmit={handleJoin} className="animate-slide-up">
            <div className="glass rounded-2xl p-5 border-hunt-border">
              ...
              <label className="block text-xs text-gray-500 uppercase tracking-widest mb-2 mt-4 font-body">
                Your Name
              </label>
              <input
                type="text"
